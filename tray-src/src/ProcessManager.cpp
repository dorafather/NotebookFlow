...
bool CProcessManager::Launch(const std::wstring& exePath, const std::wstring& workingDir)
{
    ...
    // 이 인스턴스로 두 번째 Launch()가 걸리는 경우(죽은 프로세스를
    // 재기동) 이전 프로세스의 핸들을 닫지 않고 m_pi를 덮어쓰면 새 프로세스가
    // 뜰 때마다 핸들이 2개씩 누수된다 - 재사용 전에 먼저 정리한다.
...
{
    // TerminateProcess(m_pi.hProcess)는 이 핸들이 가리키는
